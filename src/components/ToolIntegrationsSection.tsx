...
const ToolIntegrationsSection = () => {
  const { title, items } = siteContent.featuresPage.features[0]; 
        ...
        <div className="text-center space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((feature, index) => (
              <Card key={index} className="border-transparent shadow-sm hover:shadow-md transition-shadow duration-300 p-6" style={{ backgroundColor: 'hsl(var(--card-light-green))' }}>
                <CardContent className="p-0 text-center space-y-4">
                  {/* Icon */}
                  <div className="flex justify-center mb-4">
                    <div className="w-20 h-20 flex items-center justify-center">
                      <IconRenderer icon={feature.icon} className="w-16 h-16" />
                    </div>
                  </div>
