...
const OnboardingModal = ({ open, onComplete, onboardingData }: OnboardingModalProps) => {
            ...
            if (signupError) {
              ...
              if (signupData?.userId) {
                ...
                if (signupData.onboardingData) {
                  const { progress, draft } = signupData.onboardingData;
    ...
    const checkAuthAndLoadData = async () => {
      ...
      if (userId) {
        ...
        if (onboardingData?.onboardingData) {
          const { progress, draft } = onboardingData.onboardingData;
  ...
  const loadOnboardingData = async (email: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('get-lead-onboarding-data', {
        body: { userId }
      });
  ...
  const specialtyOptions = {
    "Restaurant": [
      "Italian cuisine", "Mexican food", "Asian fusion", "Steakhouse", 
      "Seafood", "Mediterranean", "French cuisine", "Indian cuisine",
      "Thai food", "Sushi & Japanese", "American cuisine", "Farm-to-table"
    ],
    "Café": [
      "Coffee & pastries", "Specialty coffee", "Brunch", "Light meals",
      "Artisan coffee", "Tea house", "Breakfast & lunch", "Organic café"
    ],
    "Fast Food": [
      "Burgers & fries", "Pizza", "Sandwiches", "Fried chicken",
      "Tacos & burritos", "Hot dogs", "Quick serve Asian", "Healthy fast food"
    ],
    "Bakery": [
      "Artisan breads", "Pastries & desserts", "Wedding cakes", "Cupcakes",
      "French pastries", "Sourdough", "Gluten-free baking", "Custom cakes"
    ],
    "Food Truck": [
      "Gourmet burgers", "Street tacos", "BBQ", "Fusion cuisine",
      "Ice cream", "Sandwiches", "Ethnic street food", "Breakfast truck"
    ],
    "Catering": [
      "Corporate catering", "Wedding catering", "Event catering", "Lunch delivery",
      "Party platters", "Holiday catering", "Specialty diets", "Buffet style"
    ],
    "Other": [
      "Fine dining", "Food delivery", "Meal prep", "Ghost kitchen",
      "Pop-up restaurant", "Bar & grill", "Buffet", "Food hall"
    ]
  };

  const challengeOptions = [
    { name: "Rising ingredient costs", icon: DollarSign },
    { name: "Low customer traffic", icon: Users },
    { name: "Staff shortages & schedules", icon: Calendar },
    { name: "Menu profitability", icon: TrendingUp },
    { name: "Food waste", icon: Trash2 },
    { name: "Lack of time for marketing", icon: Megaphone }
  ];
  ...
  const handleChallengeToggle = (challengeName: string) => {
    setFormData(prev => {
      const challenges = prev.challenges.includes(challengeName)
        ? prev.challenges.filter(c => c !== challengeName)
        : prev.challenges.length < 3
        ? [...prev.challenges, challengeName]
        : prev.challenges;
  ...
  const handleToolToggle = (toolName: string) => {
    setFormData(prev => {
      const selectedTools = prev.selectedTools.includes(toolName)
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const { error: onboardingError } = await supabase.functions.invoke('signup-complete', {
        body: {
          userId: userId,
  ...
  const handleContinue = async () => {
    ...
    try {
      if (currentStep <= 6) {
        const { error } = await supabase.functions.invoke('onboarding-update', {
          body: {
            userId: userId,
            step: currentStep,
            payload: payload
        ...
        if (currentStep === 6) {
          ...
          const { error: completeError } = await supabase.functions.invoke('signup-complete', {
            body: {
              userId: userId,
  ...
  const renderStep4 = () => {
    ...
    const placeholderText = currentSpecialties.length > 0 
      ? `e.g., ${currentSpecialties.slice(0, 3).join(', ')}`
        ...
        {challengeOptions.map((challenge) => {
          ...
          return (
            <Button
              key={challenge.name}
              variant="outline"
              className={`h-auto p-4 justify-start text-left transition-all hover:shadow-md hover:border-primary/30 ${
    ...
    <div className="space-y-6">
      <div className="grid gap-4 min-h-[600px] overflow-y-auto">
        {productRecommendations.map((product, index) => (
          <Card 
            key={index} 
            className={`border border-border cursor-pointer transition-all hover:shadow-md hover:border-primary/30 ${
              formData.selectedTools.includes(product.name) ? 'bg-primary/5 border-primary/50' : ''
            }`}
            onClick={() => handleToolToggle(product.name)}
          >
            <CardContent className="p-4">
              <div className="flex items-center gap-4">
                <Checkbox 
                  checked={formData.selectedTools.includes(product.name)}
                  className="flex-shrink-0 pointer-events-none"
                />
                <div className="flex-shrink-0 flex justify-center items-center">
                  {product.name === "MarketMan" && (
                    <img 
                      src="/lovable-uploads/26e791bd-7c02-4565-ba0a-dd5e6dc1dd01.png" 
                      alt="MarketMan logo" 
                      className="w-16 h-16 object-contain"
                    />
                  )}
                  {product.name === "OpenTable" && (
                    <img 
                      src="/lovable-uploads/aba171dd-c0ba-47e8-8ac7-7c1b67cbc134.png" 
                      alt="OpenTable logo" 
                      className="w-16 h-16 object-contain"
                    />
                  )}
                  {product.name === "7shifts" && (
                    <img 
                      src="/lovable-uploads/d8b46a21-b137-4f87-8227-f3ac55612d21.png" 
                      alt="7shifts logo" 
                      className="w-16 h-16 object-contain"
                    />
                  )}
                  {(product.name === "Toast - Menu Upsell" || product.name === "Toast - Food Waste Insights") && (
                    <img 
                      src="/lovable-uploads/806ab93c-b103-4da0-869d-b2dfa9b1d916.png" 
                      alt="Toast logo" 
                      className="w-16 h-16 object-contain"
                    />
                  )}
                  {product.name === "SpotHopper" && (
                    <img 
                      src="/lovable-uploads/f5df8e4d-3189-4a31-8f9e-899082910b78.png" 
                      alt="SpotHopper logo" 
                      className="w-16 h-16 object-contain"
                    />
                  )}
                  {!["MarketMan", "OpenTable", "7shifts", "Toast - Menu Upsell", "Toast - Food Waste Insights", "SpotHopper"].includes(product.name) && (
  ...
  const getStepTitle = () => {
    switch (currentStep) {
      ...
      case 2: return "What’s your business called?"; 
      case 3: return "Where is your business located?";
      case 4: return "What do you serve or specialize in?";
      case 5: return "Your Business Challenges";
      case 6: return "Boost Your Restaurant’s Profits with the Right Tools";
  ...
  const getStepSubtitle = () => {
    switch (currentStep) {
      case 1: return "Tell us how we should greet you";
      case 2: return "Let's start with the essentials.";
      case 3: return "Tell us where your business operates.";
      case 4: return "Tell us what makes your business unique.";
      case 5: return "Select up to 3 challenges";
      case 6:
          ...
          <div className="flex flex-col items-center justify-center min-h-[200px] space-y-6">
            <div className="w-full max-w-xs">
              <div className="relative h-1 bg-muted rounded-full overflow-hidden">
                <div 
                  className="absolute top-0 left-0 h-full bg-gradient-to-r from-muted-foreground to-orange-500 rounded-full transition-all duration-100 ease-out"
                  style={{ 
                    width: `${progress}%`,
